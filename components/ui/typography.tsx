...
export const Typography = React.forwardRef<HTMLParagraphElement, TypographyProps>(
  ({ variant = 'p', clr, className = '', href = '', isExternal = false, ...props }, ref) => {
    if (variant === 'link')
      return (
        <Link
          {...props}
          href={href}
          className={cn(typographyVariants({ variant, clr, className }))}
    ...
    return (
      <Comp className={cn(typographyVariants({ variant, clr, className }))} ref={ref} {...props}>
